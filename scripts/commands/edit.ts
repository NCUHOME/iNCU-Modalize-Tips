...
import { parsePagesFile, readMeta, writeMeta } from '../lib/pages';
...
export async function editInCat() {
  ...
  if (cat.pages.length === 0) { console.log(pc.yellow('⚠️ 该分类下无页面')); return; }
  ...
  const nt = await text({ message: '标题（留空不变）:', placeholder: cur.title });
  if (isCancel(nt)) return;

  const nd = await text({ message: '描述（留空不变）:', placeholder: cur.desc || '(空)' });
  ...
  if (title === cur.title && desc === cur.desc) { console.log(pc.dim('  无变更')); return; }
