...
import { deleteFromManage, deletePage, deleteCategory } from './commands/delete';
...
async function manageMenu() {
  while (true) {
    ...
    switch (action) {
      case 'toggle':  await toggleInCat(); break;
      case 'edit':    await editInCat(); break;
      case 'sort':    await sortInCat(); break;
      case 'del-pg':  await deleteFromManage(); break;
      case 'del-cat': await deleteCategory(); break;
    }
  }
}
...
async function main() {
  ...
  while (true) {
    const action = await select<string>({
      message: '请选择操作:',
      options: [
        { value: 'add-page',   label: '📝 新增页面',   hint: '在已有分类下创建新页面' },
        { value: 'add-cat',    label: '📁 新增分类',   hint: '创建新的页面分类' },
        { value: 'manage',     label: '📋 管理',       hint: '启停 / 编辑 / 排序 / 删除' },
        { value: 'migrate',    label: '🔀 迁移页面',   hint: '将页面移到另一个分类' },
        { value: 'health',     label: '🩺 健康检查',   hint: '扫描孤立文件 & 缺失元数据' },
        { value: 'exit',       label: '退出' },
      ],
    });
    ...
    switch (action) {
      case 'add-page': await addPage(); break;
      case 'add-cat':  await addCategory(); break;
      case 'manage':   await manageMenu(); break;
      case 'migrate':  await migratePage(); break;
